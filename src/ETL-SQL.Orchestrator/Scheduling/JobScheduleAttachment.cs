...
namespace ETL_SQL.Orchestrator.Scheduling
{
    ...
    public static class JobScheduleAttachment
    {
        ...
        {
            // The schedule name is resolved in the job's own tenant: a job can only ever be attached
            // to a schedule its tenant owns, so there is no way to reach across the boundary here.
            ...
            return await catalog.AddJobScheduleAsync(jobId, schedule.Id, nextRun);
        }
    }
}
