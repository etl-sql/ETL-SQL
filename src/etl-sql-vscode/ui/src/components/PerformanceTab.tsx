...
export const PerformanceTab: React.FC<PerformanceTabProps> = ({ metrics }) => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden font-mono text-[12px]">
      ...
      {/* Content wrapper with overall vertical scrolling */}
      <div className="flex-1 overflow-y-auto scrollbar-fancy">
        {/* Top section: two columns filling full width */}
        <div className="flex flex-row border-b border-[var(--border)]">
        ...
        {/* Bottom: detailed execution profile — full width */}
        <div className="flex flex-col">
          <div className="px-2 py-1 border-b border-[var(--border)] bg-[var(--vscode-editorGroupHeader-tabsBackground,var(--bg-darker))] sticky top-0 z-10">
            <span className="text-[11px] font-semibold text-[var(--muted)]">Execution Profile</span>
          </div>
          <div>
            <table className="w-full border-collapse">
              <thead className="sticky top-[24px] bg-[var(--vscode-editorGroupHeader-tabsBackground,var(--bg-darker))] z-10">
