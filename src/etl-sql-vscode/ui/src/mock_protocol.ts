...
export const mockTrace: ProtocolMessage[] = [
    ...
    { type: 'reportManifest', source: 'mock:///report.rptsql', title: 'Sales Performance', description: 'Annual breakdown of sales by region and category', builtAt: new Date().toISOString(), parameters: { '@region': 'All' }, visuals: [
        { name: 'Revenue by Region', visualType: 'BAR', columns: ['region', 'revenue'], rows: [['North', 500000], ['South', 300000], ['East', 450000], ['West', 250000]], actions: [], interaction: { key: 'region', valueKey: 'revenue', select: 'MULTIPLE', effect: 'HIGHLIGHT', highlight: 'PROPORTIONAL' }, options: { 'mapping:x': 'region' } },
        { name: 'Category Breakdown', visualType: 'BAR', columns: ['category', 'revenue'], rows: [['Apparel', 541905.80], ['Home', 320000], ['Electronics', 780000]], actions: [], interaction: { key: 'category', valueKey: 'revenue', select: 'MULTIPLE', effect: 'HIGHLIGHT', highlight: 'PROPORTIONAL' }, options: { 'mapping:x': 'category' } },
