    ...
    public class SchedulerService
    {
        ...
        private async Task RunAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                try 
                {
                    await Task.Delay(TimeSpan.FromSeconds(30), ct); // Check every 30 seconds
                }
        ...
        private async Task ExecuteJobAsync(JobDefinition job)
        {
            ...
            try
            {
                ...
                // Parse the script
                var lexer = new Lexer(job.Script);
                var tokens = lexer.Tokenize();
                var parser = new Parser(tokens, job.Script);
                ...
                if (historyId > 0)
                {
                    await _store.LogJobEndAsync(historyId, "SUCCESS", rowsProcessed: evaluator.RowsProcessed);
                }
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                {
                    _logger.LogError(ex, "Failed to update last run for {JobName}.", job.Name);
                }
            }
        }
        ...
        private DateTime CalculateNextRun(JobDefinition job)
        {
            ...
            {
                // If it's a daily job with AT time
                if (unit == "DAY")
                {
