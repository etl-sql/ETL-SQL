    ...
    SmtpPasswordProtector  pwdProtector) : ControllerBase
{
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateSubscriptionRequest req)
    {
        ...
        if (scriptNeedsRewrite && !string.IsNullOrEmpty(sub.ScriptPath))
        {
            ...
            {
                smtp = await db.SmtpConnections.FirstOrDefaultAsync(c => c.Alias == sub.SmtpAlias);
            }
            GenerateJobScript(sub, sub.Report, smtp, sub.Recipients, null, newParams);
        }
        ...
        Dictionary<string, string>? parameters)
    {
        ...
        // Inject parameter DECLARE statements so the report script picks them up at execution time.
        ...
        if (sub.Format != SubscriptionFormat.Link)
        {
            ...
            sb.AppendLine($"EXPORT REPORT '{report.ScriptPath.Replace("\\", "/")}' FORMAT {formatName} TO '{outputPath.Replace("\\", "/")}';");
            sb.AppendLine();
            ...
            sb.AppendLine($"    ATTACH '{outputPath.Replace("\\", "/")}'");
        ...
        SubscriptionFormat? newFormat)
    {
        ...
        if (parameters is { Count: > 0 })
        {
            var setLines = parameters.Select(p =>
            {
                var k = p.Key.StartsWith("@") ? p.Key : "@" + p.Key;
                return $"DECLARE {k} STRING = '{Esc(p.Value)}';";
            }).ToList();
            setLines.Add("");
