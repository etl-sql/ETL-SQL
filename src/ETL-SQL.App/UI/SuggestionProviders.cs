...
namespace ETL_SQL.UI
{
    ...
    public class AliasColumnProvider : ISuggestionProvider
    {
        public async Task<IEnumerable<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            ...
            catch (Exception ex) { Logger.Verbose($"[AliasColumnProvider] Column resolution error: {ex.Message}"); }
    ...
    public class FilePathProvider : ISuggestionProvider
    {
        public Task<IEnumerable<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            try
            {
                ...
                return Task.FromResult(ETLSuggestEngine.GetFileSuggestions(context.Prefix).Select(s => new Suggestion(s, SuggestionType.FilePath)));
            }
            catch (Exception ex) { Logger.Verbose($"[FilePathProvider] File path suggestion error: {ex.Message}"); return Task.FromResult(Enumerable.Empty<Suggestion>()); }
        }
    }
            ...
            try
            {
                ...
                foreach (Match m in connMatches)
                {
                    ...
                    if (connector != null)
                    {
                            ...
                            if (context.Prefix.StartsWith($"{connName}.", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if (tablePref.Contains("."))
                                {
                                    ...
                                    if (tables.Any(t => t.Equals(tablePref, StringComparison.OrdinalIgnoreCase)))
                                    {
                                        ...
                                        catch (Exception ex) { Logger.Verbose($"[DatabaseSchemaProvider] GetColumnsAsync error for {connName}.{tablePref}: {ex.Message}"); }
                                    }
                        ...
                        catch (Exception ex) { Logger.Verbose($"[DatabaseSchemaProvider] Connector error for '{connName}': {ex.Message}"); }
                    }
                }
            }
            catch (Exception ex) { Logger.Verbose($"[DatabaseSchemaProvider] Schema discovery error: {ex.Message}"); }
            return results;
        }
    }
    ...
    public class WithClauseProvider : ISuggestionProvider
    {
        public Task<IEnumerable<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            ...
            catch (Exception ex) { Logger.Verbose($"[WithClauseProvider] Option discovery error: {ex.Message}"); }
            return Task.FromResult<IEnumerable<Suggestion>>(results);
        }
    }
    ...
    public class ContextAwareProvider : ISuggestionProvider
    {
        public async Task<IEnumerable<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            ...
            try
            {
                ...
                else if (prevWord == "FROM" || prevWord == "JOIN" || prevWord == "INTO" || prevWord == "UPDATE")
                {
                    ...
                    foreach (var conn in context.Connections.Values)
                    {
                        ...
                        {
                            try { results.AddRange((await db.GetTablesAsync()).Select(t => new Suggestion(t, SuggestionType.Table))); } catch (Exception ex) { Logger.Verbose($"[ContextAwareProvider] GetTablesAsync error: {ex.Message}"); }
                            try { results.AddRange((await db.GetViewsAsync()).Select(v => new Suggestion(v, SuggestionType.Table))); } catch (Exception ex) { Logger.Verbose($"[ContextAwareProvider] GetViewsAsync error: {ex.Message}"); }
                        }
                    }
            ...
            catch (Exception ex) { Logger.Verbose($"[ContextAwareProvider] Context awareness error: {ex.Message}"); }
            return results;
        }
    }
    ...
    public class VariableProvider : ISuggestionProvider
    {
        public Task<IEnumerable<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            ...
            catch (Exception ex) { Logger.Verbose($"[VariableProvider] Variable discovery error: {ex.Message}"); return Task.FromResult(Enumerable.Empty<Suggestion>()); }
        }
    }
    ...
    public class SuggestionEngine
    {
        ...
        public async Task<List<Suggestion>> GetSuggestionsAsync(SuggestionContext context)
        {
            ...
            foreach (var provider in _providers)
            {
                ...
                catch (Exception ex) { Logger.Verbose($"[SuggestionEngine] Provider {provider.GetType().Name} error: {ex.Message}"); }
            }

            var filtered = allSuggestions
                .Where(s => string.IsNullOrEmpty(context.Prefix) || s.Text.StartsWith(context.Prefix, StringComparison.OrdinalIgnoreCase))
                .ToList();
