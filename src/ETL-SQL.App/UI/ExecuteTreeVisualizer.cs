...
namespace ETL_SQL.UI
{
    ...
    /// Optimized for speed and low CPU overhead.
    /// </summary>
    public class ExecuteTreeVisualizer
    {
        ...
        public async Task RenderLiveAsync(CancellationToken ct)
        {
            ...
            {
                // In redirected or non-interactive environments, we skip the live TUI
                // but nodes still process metrics in the background.
                return;
            }
            ...
            await AnsiConsole.Live(CreateRenderable())
                ...
                .StartAsync(async ctx =>
                {
                    while (!ct.IsCancellationRequested)
                    {
                        ...
                        await Task.Delay(100, ct); // 10Hz throttle
                    }
                });
        }
