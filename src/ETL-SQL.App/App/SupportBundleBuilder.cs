...
namespace ETL_SQL.App
{
    ...
    {
        // Redaction moved to ETL_SQL.Core.Common.SupportBundleRedactor when the Portal gained its
        // own online-safe bundle: two hosts now produce support material, and redaction that is
        // almost the same in two places is worse than none — it yields two artifacts that look
        // equally safe and are not. These forwarders keep the call sites here unchanged.

        private const string RedactedMarker = ETL_SQL.Core.Common.SupportBundleRedactor.RedactedMarker;

        /// <inheritdoc cref="ETL_SQL.Core.Common.SupportBundleRedactor.IsSecretKey"/>
        internal static bool IsSecretKey(string key) =>
            ETL_SQL.Core.Common.SupportBundleRedactor.IsSecretKey(key);

        /// <inheritdoc cref="ETL_SQL.Core.Common.SupportBundleRedactor.RedactConfigJson"/>
        internal static string RedactConfigJson(string json) =>
            ETL_SQL.Core.Common.SupportBundleRedactor.RedactConfigJson(json);

        /// <inheritdoc cref="ETL_SQL.Core.Common.SupportBundleRedactor.RedactDiagnosticText"/>
        internal static string RedactDiagnosticText(string? text) =>
            ETL_SQL.Core.Common.SupportBundleRedactor.RedactDiagnosticText(text);
        ...
        private static async Task WriteManifestAsync(string staging)
        {
            var manifest = new JsonObject
            {
                ...
                ["machineName"] = ETL_SQL.Core.Common.SupportBundleRedactor.RedactedValueMarker,
        ...
        private static async Task WriteRedactedConfigAsync(string staging)
        {
            ...
            try
            {
                redacted = RedactConfigJson(await File.ReadAllTextAsync(appSettings));
            }
            catch (Exception ex)
            {
