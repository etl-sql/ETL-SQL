...
-- Capture help text for programmatic use
HELP SELECT INTO #docs;
SELECT line FROM #docs WHERE line LIKE '%INTO%';
```
...
- Output can be captured with `INTO #table` — each row contains a `line` column with one line of help text.
