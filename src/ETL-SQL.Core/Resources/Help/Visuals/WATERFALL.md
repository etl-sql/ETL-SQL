...
Mappings:
  ...
  VALUE   — the change amount (positive = increase, negative = decrease)
            Mark the total bar with a special flag (see TOTAL column below)
  TOTAL   — optional boolean column; rows where TOTAL = 1 are drawn as totals
             (anchored to zero, showing the running sum)
