namespace ETL_SQL.Core.Parser;

public enum TokenType
{
    // Keywords
    CREATE, CONNECTION, ON, FILE, DATABASE, EXCEL, JSON, XML,
    MSSQL, ORACLE, POSTGRES, SNOWFLAKE, BIGQUERY, MOCKDB, ODBC, FLATFILE, GENERATE, COMPARE, DATASETS, CALENDAR, TRANSFORM,
    SELECT, FROM, WHERE, GROUP, BY, HAVING, QUALIFY, WINDOW, FILTER,
    // QUARANTINE is deliberately NOT a token type. It appears only in positions the parser matches
    // contextually (ON FAILURE QUARANTINE, REPLAY QUARANTINE), and "quarantine" is the most natural
    // name in this domain for a table or connection — reserving it broke real scripts.
    INSERT, INTO, VALUES, REPLACE, REPLAY,
    UPDATE, SET,
    DELETE, TRUNCATE, PIVOT, UNPIVOT, MERGE, USING, MATCHED, SOURCE,
    COPY, MOVE, COMPRESS, DECOMPRESS, SEND, RECEIVE, EMAIL, ADD, COLUMN, EXPORT,
    DOCKER, CLOSE,
    DECLARE,
    AND, OR, NOT, AS, IS,
    IN, LIKE, ILIKE, ESCAPE,
    CASE, WHEN, THEN, ELSE, END,
    TABLE, IF, EXISTS, IDENTITY, DEFAULT, DROP, WITH, WITHIN,
    DISTINCT, TOP, PERCENT, TIES, LIMIT, OFFSET, FETCH, FIRST, NEXT, ONLY, ROWS, ROW, NO,
    BEGIN, WHILE, FOR, FOREACH, EACH, TO, STEP, ZONE, TIME,
    TRY, CATCH, THROW,
    PRINT, RAISEERROR, ASSERT, EXEC, EXECUTE,
    RETURN, BREAK, CONTINUE, GO,
    PATH, ROOT, AUTO, RAW, EXPLICIT, ELEMENTS, INDEX, UNIQUE, EXPLAIN, SEMI, ANTI, PROCEDURE, FUNCTION, RETURNS, REQUIRE,
    PRIMARY, KEY, FOREIGN, CHECK, REFERENCES, CONSTRAINT, CLEAR, SESSION, SESSIONS,
    TRANSACTION, TRAN, COMMIT, ROLLBACK, ALTER, AT, RECURSIVE,
    DIRECTORY, RENAME, ENCRYPT, DECRYPT, DIRECTORY_CONTENTS,
    COPY_FILE, MOVE_FILE, RENAME_FILE, DELETE_FILE, COMPRESS_FILE, DECOMPRESS_FILE, ENCRYPT_FILE, DECRYPT_FILE,
    CREATE_DIRECTORY, DELETE_DIRECTORY, RENAME_DIRECTORY, MOVE_DIRECTORY,
    COPY_DIRECTORY, DELETE_DIRECTORY_CONTENTS,
    COMPRESS_DIRECTORY, DECOMPRESS_DIRECTORY, ENCRYPT_DIRECTORY, DECRYPT_DIRECTORY,
    SEND_FILE, RECEIVE_FILE, FILE_SEND, FILE_RECEIVE, SFTP, FTP_CONN, AZURE_BLOB,
    DATETIMEFROMPARTS, TIMEFROMPARTS, DATETIMEOFFSETSFROMPARTS, HASHBYTES, NEWID, NEWSEQUENTIALID, CHECKSUM, BINARY_CHECKSUM,
    STUFF, STRING_ESCAPE, STRING_SPLIT, ASCII, UNICODE, CHAR, PATINDEX, STR, QUOTENAME, TRANSLATE, DATALENGTH, TO_STR, REPLICATE, TRY_CAST,
    SIN, COS, TAN, ASIN, ACOS, ATAN, ATAN2, SIGN,
    WAIT, WAITFOR, DELAY,
    HELP, TYPE, TARGET, TRUE, FALSE,
    PASSWORD, SHOW_PASSWORD, SHOW_SECRETS, ALLOW_PLAINTEXT_SECRETS, NO_SAVE_SENSITIVE, NO_SAVE_CONNECTION, CONNECTION_ENCRYPTION, OUTPUT, INPUT, REQUIRED, PARALLEL, RUN, SCRIPT, USE, START, STOP, PAUSE, OFF, PROFILE, PROFILING, SHOW, VERSION, KILL,
    BULK, LOAD, BATCHSIZE, MAXERRORS, FIELDTERMINATOR, ROWTERMINATOR, FIRSTROW, DATA_SOURCE,
    NULL, YEAR, MONTH, DAY, HOUR, MINUTE, SECOND, INCLUDE_NULL_VALUES, WITHOUT_ARRAY_WRAPPER,
    JOIN_SPILL_THRESHOLD, EXTERNAL_HASH_PARTITIONS, EXTERNAL_SORT_CHUNK_SIZE, WINDOW_SPILL_THRESHOLD, TEMP_TABLE_SPILL_THRESHOLD,
    MAX_RECURSIVE_DEPTH, MAX_IN_MEMORY_BATCHES, FOREACH_PAGE_SIZE, MAX_MESSAGES, MAX_FILE_OPERATIONS,
    MAX_PARALLEL_DEGREE, MAX_STRING_RESULT_SIZE, REGEX_MATCH_TIMEOUT, MAX_LAST_RESULT_ROWS, MAX_GENERATE_ROWS, MAX_SMTP_EMAILS_PER_SCRIPT, MAX_INTERNAL_OPERATIONS,
    OPERATOR_MEMORY_GRANT, CONNECTION_PREVIEW_LIMIT,
    SPILL_ENCRYPTION, SPILL_COMPRESSION,
    MAX_GROUPING_SETS, SET_CUBE_LIMIT,
    MAX_SESSION_SIZE,
    PERSIST,
    TELEMETRY,
    TRUNCATE_STRING,
    SKIP_ERROR,
    LINEAGE, LINEAGE_NAMESPACE, LINEAGE_IMPORT_CATALOG, SEND_EMAIL, SUBJECT, BODY, ATTACH, CC, BCC,
    JOB, SCHEDULE, EVERY, HISTORY, JOBS, LINT, SSH_KEY_PAIR, PGP_KEY_PAIR, WHAT_IF, SYSDATE, TOOL,
    CONNECTIONS, TABLES, COLUMNS, TAGS, TAG, VALUE, BITS, ALGORITHM, PASSPHRASE, COMMENT, PGP_KEY,
    SUBSTRING, POSITION, OVERLAY, EXTRACT, TRIM, PLACING, LEADING, TRAILING, BOTH,
    CHARACTER_LENGTH, CHAR_LENGTH, OCTET_LENGTH,
    VARIABLES, LOCAL, ANALYZE, SAFE, ZONES,
    SETS, BANG,     // SETS keyword and ! prefix for set names
    ROLLUP, CUBE, GROUPING,   // GROUP BY extensions
    RANGE, GROUPS, BETWEEN, PRECEDING, FOLLOWING, UNBOUNDED, CURRENT, EXCLUDE, OTHERS,
    UNTIL,
    CONFIG,
    WEEK_START_DAY,
    INTERACTIVE_MODE,
    SCRIPT_HASH_POLICY,
    CASE_SENSITIVE, DATA_QUALITY_DRY_RUN,
    VERIFY, INTEGRITY, SYNC, UNLOCKED, ENCODING, FROM_ENCODING, TO_ENCODING, LIMIT_TYPE, LIMIT_VALUE, HASH_FILE, EXPECTED_HASH, DELETE_EXTRA, FILES, SPLIT, TIMEOUT, POLL_INTERVAL_MS, CONVERT,
    LF, CR, CRLF, TAB,
    SHAREPOINT, ACTIVE_DIRECTORY, SQLITE, S3, MONGODB, KAFKA,

    // Joins
    JOIN, INNER, LEFT, RIGHT, OUTER, FULL, CROSS, APPLY, LATERAL, ASOF, HASH, LOOP, FUZZY, KEEP,

    // Set Operations
    UNION, ALL, EXCEPT, INTERSECT,

    // Functions
    UPPER, LOWER, CONCAT, CAST, TRUNC, GETDATE,
    CURRENT_TIMESTAMP, CURRENT_DATE, CURRENT_TIME, FORMAT,
    OVER, PARTITION, ORDER, ASC, DESC,
    SUM, AVG, COUNT, MIN, MAX,
    SEQUENCE, RANDOM, RANDOM_INT, RANDOM_DECIMAL,

    // Data types
    INT, INTEGER, BIGINT, SMALLINT, TINYINT, BIT, BOOLEAN, BOOL,
    DECIMAL, NUMERIC, MONEY, SMALLMONEY, FLOAT, REAL, DOUBLE,
    DATE, DATETIME2, SMALLDATETIME, DATETIMEOFFSET, TIMESTAMP,
    VARCHAR, VARCHAR2, NCHAR, NVARCHAR, TEXT, NTEXT, BINARY, VARBINARY, IMAGE, STRING,
    MINMAX, SENSITIVE, SECRET, RELDATE,
    UNIQUEIDENTIFIER, UUID, GUID, GEOMETRY, GEOGRAPHY, HIERARCHYID,
    VARIANT, SQL_VARIANT, ANY, MARKDOWN, DESCRIBE, SAMPLE, REPEATABLE,

    IDENTIFIER,
    VARIABLE,   // @variableName
    STRING_LITERAL,
    NUMBER,
    DATETIME,   // Common parsed date types

    // Symbols / Operators
    STAR,       // *
    COMMA,      // ,
    SEMICOLON,  // ;
    DOT,        // .
    LPAREN,     // (
    RPAREN,     // )
    LBRACKET,   // [
    RBRACKET,   // ]
    EQUALS,     // =
    LESS_THAN,  // <
    GREATER_THAN, // >
    LESS_EQUALS, // <=
    GREATER_EQUALS, // >=
    NOT_EQUALS, // != or <>
    REGEX_MATCH, // ~
    REGEX_IMATCH, // ~*

    PLUS,       // +
    MINUS,      // -
    SLASH,      // /
    MODULO,     // %
    LSHIFT,     // <<
    RSHIFT,     // >>
    QUESTION,   // ?
    PARAMETER,  // ? or ?n
    DOUBLE_QUESTION, // ?? (null-coalescing shorthand; lowers to COALESCE at parse time)
    ARROW,           // => (arrow conditional; chain lowers to CASE WHEN at parse time)
    JSON_ARROW,      // -> (JSON field/element access; lowers to JSON_GET at parse time)
    JSON_ARROW_TEXT, // ->> (JSON field/element access as text; lowers to JSON_GET_TEXT)
    COLUMN_TAG, // /*@d: ... */

    // ── Report-SQL tokens (Phase 9A) ───────────────────────────────────
    // All are non-reserved: only treated as keywords inside CREATE VISUAL /
    // CREATE DASHBOARD / CREATE PAGE context.
    VISUAL, DASHBOARD, PAGINATED, THEME, LAYOUT, REFRESH, VISIBLE,
    BAR, LINE, PIE, GAUGE, RADAR, SCATTER, HEATMAP, TABLE_VISUAL, FUNNEL, WATERFALL, FORMATTING,
    DONUT, HBAR, BOXPLOT, TREEMAP, COLORS, BUBBLE, CANDLESTICK, GANTT,
    SANKEY, SUNBURST, NETWORK, TRELLIS, MATRIX,
    BINDING, DIRECT, GATEWAY,
    BOTTOM,  // position keyword (TOP/LEFT/RIGHT already registered via LanguageMetadata)
    TITLE, SUBTITLE, DESCRIPTION,
    MAPPINGS, OPTIONS, ACTIONS, STRUCTURE, MAP, SERIES,
    SLICER, CARD, ON_CLICK, DRILL_DOWN, DRILL_IN, DRILL_REPORT, SET_PARAMETER, ON_CHANGE, ON_SUBMIT,
    TTL, KEYFILE, X_AXIS, Y_AXIS,
    PAGE, PAGES, DATASET,
    REPORT,
    STYLE, CONTAINER, BOX, SCROLL, NAVIGATION, COMBO, NAV_TAB, BUTTON, LINK_NAV,
    DATEPICKER, RELDATEPICKER, SLIDER, MULTISELECT, SEARCH,
    CHECKBOX, TEXTBOX, NUMBERBOX, LABEL_POSITION, DECIMALS, PLACEHOLDER, CONTENT,
    TOOLTIP, BACK,
    TEMPLATE, TEMPLATE_PATH,
    SUMMARY, GRAND_TOTAL, GRAND_TOTAL_ROW, GRAND_TOTAL_COLUMN, SUMMARIZE_ROW, SUMMARIZE_COLUMN,
    GRID, NONE, HEADER, FOOTER, DATA_LABELS, DATA_LABELS_POSITION, FONT_FAMILY, FONT_WEIGHT,
    FONT_SIZE, CENTER, INSIDE, INSIDE_TOP, INSIDE_BOTTOM, INSIDE_LEFT, INSIDE_RIGHT,
    INSIDE_TOP_LEFT, INSIDE_TOP_RIGHT, INSIDE_BOTTOM_LEFT, INSIDE_BOTTOM_RIGHT,
    GAUGE_STYLE, SHOW_NO_DATA_PLACEHOLDER,
    INTERACTIONS, HIGHLIGHT, CLEAR_FILTERS,
    CSS, JS, FAVICON, LOGO, BACKGROUND, NAV_OVERRIDE, HEAD,
    RUN_SCRIPT, SET_UI_STATE, APPLY_PARAMETERS, NAVIGATE_PAGE, REFRESH_VISUALS, APPLY_BOOKMARK,
    RESET_PARAMETERS, OPEN_URL, SHOW_MODAL, HIDE_MODAL,
    BOOKMARK,
    ICON, PINNABLE, COLLAPSIBLE, BADGE,
    PRINT_LAYOUT, PAGE_LAYOUT, PAGE_SIZE, ORIENTATION, MARGINS, MARGIN, UNITS, UNIT, OVERFLOW, PAGE_BREAK_BEFORE, PAGE_BREAK_AFTER, KEEP_TOGETHER, EXCLUDE_FROM_PRINT,
    FREEZE, TOTAL_POSITION, DEFAULT_SORT,
    ON_LOAD, MOBILE_LAYOUT, BREAKPOINT, ANNOTATIONS, POINT, COORD, ACTIVE_STYLE,
    SEGMENT_STYLE, LINE_DASH,
    EXPECT, SCHEMA,   // EXPECT SCHEMA statement


    // ── Portal admin tokens (Phase 10) ─────────────────────────────────
    // Non-reserved: treated as keywords only inside EXECUTE portal BEGIN…END context.
    USER, FOLDER, GRANT, REVOKE, PUBLISH, SUBSCRIPTION,
    DISCONNECT, TOKENS, RESTART, SHUTDOWN, PORTAL,
    REBUILD, SMTP, ACTIVE, DELIVER, RECIPIENTS, INTERVAL,
    ENABLE, DISABLE, PDF, CSV,
    FAVORITE, UNFAVORITE, CATALOG, PERMISSIONS, EFFECTIVE, USAGE, METRICS, VALIDATE,
    SHARE, LINK, EXPIRES,
    EMBED, TOKEN, SAVED, VIEW, ALERT, TRIGGER,


    // ── Overlay tokens (Phase 9F) ──────────────────────────────────────
    OVERLAYS, GOAL, AVERAGE, MOVING_AVG, LINEAR, EXPONENTIAL, LOGARITHMIC, POLYNOMIAL, POWER,
    SOLID, DASHED, DOTTED, COLOR,
    FONT_COLOR, DATA_BAR, COLOR_SCALE, ICON_SET,
    HYPERLINK, SPARKLINE,

    COLON, GOTO,
    EOF         // End of file / string
}
