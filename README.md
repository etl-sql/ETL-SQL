...
- **Over 11,000 SQL-correctness checks.** The engine is exercised against the active portion of the industry-standard SQLite [`sqllogictest`](https://www.sqlite.org/sqllogictest/) corpus — 40 files containing 9,300+ queries and 2,100+ statements — as a dedicated lane that compares ETL-SQL's output against a reference SQL implementation (the remaining ~2.1 million index-optimization checks are excluded because they rely on indexing physical tables, which ETL-SQL only supports on in-memory `#temp` tables).
