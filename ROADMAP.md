...
- [ ] **Smart Snippets (Schema-Aware Code Generator):** Extend the Language Server's autocomplete functionality to auto-generate complex query boilerplate. See the detailed design spec in [SmartSnippetsSpec.md](Docs/Design/SmartSnippetsSpec.md). When a developer triggers a smart snippet (e.g., `/merge <src> <dest>` or `/upsert <src> <dest>`), the editor launches a schema-mapping wizard:
  ...
  - **Optimized Templates:** Generate optimized `MERGE` blocks (checking if data actually changed before updating) or split `UPDATE`/`INSERT` queries for performance-critical targets.
- [ ] **Unified Notebook & Script Execution:** Unify the `.etlnb` notebook REPL engine with plain-text `.etlsql` scripts by treating top-level labels as virtual cell boundaries. See the detailed design spec in [UnifiedNotebookScriptExecution.md](Docs/Design/UnifiedNotebookScriptExecution.md). This includes:
