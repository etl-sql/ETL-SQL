...
## 16b. Outbound REST API Submission (Sink)
...
## 21. Importing Curated Lineage & Tags (Non-Standard Sources)
...
> Imports are a starting point, not a freeze: any lineage the script produces afterwards accrues on top (last-writer-wins). See [Reference/Lineage.md](Reference/Lineage.md) for the tag/lineage import grammar.

---

## 22. Fuzzy Entity Resolution (Customer De-duplication)
...
> Use `KEEP BEST 3` and `ORDER BY __score DESC` instead when you want the top few candidates per row for a human to adjudicate. See [Reference/Grammar.md](Reference/Grammar.md) §5.4 for the full `FUZZY JOIN` options and the `SIMILARITY` / `LEVENSHTEIN` / `NORMALIZE` functions.
...
## 25. Specification-Driven Vendor Feed Build
...
For a real vendor document, copy [data_spec_parser_instructions.md](data_spec_parser_instructions.md) into your AI assistant with the vendor spec. Save the returned JSON and review low-confidence fields before generating a script.
