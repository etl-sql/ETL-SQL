...
- **Page behaviour goes in a file, not in the page.** A Portal page loads
  ...
  rooted `/js/api.js` is a path no checker can resolve.
- **Shapes that cross the C#/JS boundary come from the C#.** `types/etlsql-contracts.generated.d.ts` is
  generated by reflection over the DTOs and enums; regenerate it with
  `ETLSQL_UPDATE_BROWSER_CONTRACTS=1 dotnet test tests/ETL-SQL.Portal.Tests --filter
  FullyQualifiedName~BrowserContractsGeneratorTests`. Never hand-edit it, and do not add a new
  hand-written regex test that pulls literals out of a `.js` file — declare the type instead.
...
The sync writes `// @ts-nocheck` into each generated `.js` copy's banner. A host copy is byte-identical
to the canonical file, so the browser type gate would otherwise report every finding twice, under a path
nobody would fix it at. The canonical file is the one that is checked.
