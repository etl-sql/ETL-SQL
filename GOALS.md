...
- Enforce column-level data quality rules (`EXPECT` clauses) directly on data streams, supporting native stream routing (like `QUARANTINE` and `WARN`) without pipeline rebuilds.
