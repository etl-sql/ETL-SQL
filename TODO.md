...
- [ ] **`CreateConnectionStatementHandler.cs:166–189` — Remove `Console.Error.WriteLine` debug traces**
...
- [ ] **`ExecutePushdownStatementHandler.cs:67–68` — SQL prefix stripping via `Replace()` is fragile**
...
- [ ] **`AuthController.cs:92,120` — `int.Parse(User.FindFirstValue(...)!)` throws on malformed claims**
...
- [ ] **`Program.cs:72–76` — JWT zero-byte placeholder starts the app in an insecure state**
...
- [ ] **`SecurityService.cs:24` — `GetMachineKey()` entropy is weak in containerised environments**
...
- [ ] **`SecurityService.cs` multiple bare `catch {}` blocks**
...
- [ ] **`CreateConnectionStatementHandler.cs` — Execute() has 10+ responsibilities**
...
- [ ] **`ExecutePushdownStatementHandler.cs:57–68` — No explanation of why prefix stripping exists**
...
- [ ] **`SecurityService.cs:121–137` — Extension and directory block-lists have no rationale per entry**
...
- [ ] **`SecurityService.cs:405–441` — `CheckRunawayProtection` flag semantics are opaque**
...
- [ ] **`CreateDatasetStatementHandler.cs:74–96` — No comment on failure ordering in multi-step persistence**
...
- [ ] **`ExpressionEvaluator.cs:105` — "BUG FIX" comment gives no context**
...
- [ ] **`ConnectionStringBuilder.cs` — No public type documentation**
...
- [ ] **`AlterConnectionStatementHandler.cs` — Silent `${VAR}` substitution failure**
...
- [ ] **`BulkInsertStatementHandler.cs:107–112` — Bad BATCHSIZE/MAXERRORS values throw instead of lint**
...
- [ ] **`ExecutePushdownStatementHandler.cs:27–33` — Empty pushdown body not linted**
...
- [ ] **`FlatFileDataSource` — Delimiter/row-delimiter conflict not detected**
...
- [ ] **`CreateConnectionStatementHandler.cs:114–121` — Connection string build failure loses connector context**
...
- [ ] **`BulkInsertStatementHandler.cs:162–195` — Row-by-row fallback logs only a summary**
...
- [ ] **`ExecutePushdownStatementHandler.cs:55–63` — Prefix stripping is silent**
...
- [ ] **`Orchestrator/Execution/ProcessJobExecutor.cs:124–125` — stderr always logged at Warning**
...
- [ ] **`SecurityService.cs:731–753` — `IsWithinSafeZone` is entirely silent on rejection**
...
- [ ] **`BulkInsertStatementHandler` — Column mapping edge cases untested**
...
- [ ] **`SecurityService.ValidatePath` — UNC paths and mixed separators not tested**
...
- [ ] **`ExpressionEvaluator.ResolveIdentifierFallback` — Ambiguity detection edge cases untested**
...
- [ ] **`ExecutePushdownStatementHandler` — Prefix stripping correctness untested**
...
- [ ] **`AuthController` — Auth edge cases untested**
...
- [ ] **`ExpressionEvaluator.ResolveIdentifierFallback` — O(N²) column lookup**
...
- [ ] **`CreateConnectionStatementHandler.cs:178` — `ReadBatches(10).Take(1)` batch size should be minimal**
...
- [ ] **`AuthController.cs:92,120` — Duplicate `int.Parse(User.FindFirstValue(...)!)` pattern**
...
- [ ] **`AlterConnectionStatementHandler.cs:63` — Hardcoded file-connector name array**
...
- [ ] **`ExpressionEvaluator.cs` — `qualifiedSuffixes.Where(...).ToHashSet()` allocated inside hot path**
...
- [ ] **`SecurityService.cs:5` — `Regex` fields should be `[GeneratedRegex]` or verified static**
