...
- [~] Every governed key maps to a named enforcement boundary or is removed from the policy schema as non-enforceable. *(Living audit: `GovernanceEnforcementAuditTests` classifies all 24 `GovernancePolicyRegistry` keys — 12 enforced against the execution-policy snapshot at named boundaries (incl. the mutation guardrails `Security:RequireWhatIfForDestructiveStatements` / `Security:RequireTransactionForMutations` via `MutationGuardrailPolicy.Enforce` at statement dispatch), 10 via config-precedence local/Portal boundaries, and 2 acknowledged gaps (`Security:AllowedExecutionModes`, `Security:RemoteExecutionMode`) flattened into policy values but not yet consumed. The test fails on any unclassified/stale key or gap-set drift, so a new governed key cannot ship unenforced silently. Closing the last 2 gaps (execution-mode + remote-execution enforcement boundaries) makes this `[x]`.)*
