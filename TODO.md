...
- [ ] **Data-cleanup mechanism that works from every uninstall path**  The MSI's opt-in "delete all data" checkbox only appears in the full uninstall wizard. Provide a cross-platform purge (e.g. an `etl-sql` CLI subcommand / documented script) so users on Windows ARP, Linux, and macOS can wipe reports/db/logs consistently. Ties into the installer-parity item above.
