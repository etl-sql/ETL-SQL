...
- [~] Every governed key maps to a named enforcement boundary or is removed from the policy schema as non-enforceable. *(Living audit: `GovernanceEnforcementAuditTests` classifies all 24 `GovernancePolicyRegistry` keys — 13 enforced against the execution-policy snapshot at named boundaries (mutation guardrails via `MutationGuardrailPolicy.Enforce`; `Security:AllowedExecutionModes` via `OperationPolicyBoundary.EnforceAllowedExecutionMode` at execution start), 10 via config-precedence local/Portal boundaries, and 1 acknowledged gap: `Security:RemoteExecutionMode` (no execution path produces `ScriptExecutionMode.Remote` yet, so there is no trigger point — the boundary lands with the remote-execution feature). The test fails on any unclassified/stale key or gap-set drift.)*
