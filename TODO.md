...
- [x] Implement `HOSTED` PDF export for `report serve` using the existing report runtime.
- [x] Implement `HOSTED` PDF export for ReportPortal as the current authenticated user.
- [x] Implement optional installed-browser PDF export using configured Chrome/Edge/Chromium only; do not bundle a browser.
- [x] Wire `AUTO` mode to try configured high-fidelity export and fall back to `STATIC` with a warning.
