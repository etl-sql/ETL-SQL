...
- [ ] **Apache Arrow Snapshot Format** — Migrate report snapshots to a hybrid `.zip` archive (Option A) containing a lightweight `layout.json` metadata file and separate binary `.arrow` table datasets. Integrate the `apache-arrow` JS library in the player to support lazy-loading of specific datasets as visual controls scroll into view. Reuse the core engine's existing columnar disk-spill memory structures to write Arrow IPC buffers directly from the server without transposition overhead.
