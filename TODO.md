...
- [ ] **Idempotent DDL/DML (OR REPLACE / OR IGNORE / BY NAME)** — `CREATE OR REPLACE TABLE|VIEW`, `INSERT OR IGNORE`, `INSERT OR REPLACE`, and `INSERT INTO ... BY NAME`.
  - **DONE: `CREATE OR REPLACE TABLE|VIEW`** — `CreateOrReplace` mode; SchemaManager drops the existing relational table first (temp tables overwrite); views already replace. Tests in `StmtCreateOrReplaceTests`; Grammar §10.1 + Syntax_Index.
  - REMAINING: `INSERT OR IGNORE` / `INSERT OR REPLACE` (need PK/unique conflict/upsert semantics) and `INSERT INTO ... BY NAME` (align source columns to target by name).
