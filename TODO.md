...
- [~] **Add a metadata/schema explorer, not only a file explorer** — The sidebar now has a **Schema** mode alongside Files. It lists connections → tables → columns (plus `#temp` tables), eagerly loaded from the metadata scanner. A clickable **⇄ Switch** row (mouse-friendly) and **Tab** toggle modes; a **⟳ Refresh schema** row reloads; Enter on a column (or `i` on any node) inserts the name at the cursor. Tests cover toggle, tree build, and insert. (Deferred: views as a separate group, and lazy per-node loading for very large schemas.)
