...
- [x] **Lateral column aliases** — a SELECT item may reference an alias defined by an earlier item in the same list; `ORDER BY` may reference an output alias (already worked). *(Done: bind-time inlining in `SelectStatementHandler.ApplyLateralColumnAliases`/`InlineAliases`; real source column always wins over a same-named alias; lateral-alias queries resolve locally — pushdown guarded by `HasLateralColumnAlias`. Tests: `StmtLateralColumnAliasTests`. Docs: Grammar §5.1.1, Syntax_Index, `SELECT_MODIFIERS.md`.)* Deliberately scoped to the SELECT list + ORDER BY only; alias references in WHERE/GROUP BY/HAVING/QUALIFY were intentionally **not** implemented (they conflict with standard SQL clause evaluation order).
