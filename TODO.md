...
- [ ] **FOR JSON/XML Result Materialization** — [BatchPipelineHelper.cs:L53](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.Engine/Services/BatchPipelineHelper.cs#L53): `FOR JSON` and `FOR XML` collect every streamed batch into a single `List<Row>` before formatting. Large exports can bypass spill behavior and allocate both the full row set and the final serialized string in memory.
  - *Solution*: Stream JSON/XML formatting directly from batches, or explicitly cap/document these clauses until bounded streaming serializers are available.
...
- [ ] **Subscription Catalog Search Materialization** — [SubscriptionsController.cs:L108](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.ReportPortal/Controllers/SubscriptionsController.cs#L108): Admin subscription search loads all matching subscription rows into memory before applying case-insensitive search, sorting, and paging. At 10k subscriptions this creates avoidable portal heap pressure and slow admin search responses.
...
- [ ] **Full Configuration Export Materialization** — [ConfigurationExportService.cs:L49](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.ReportPortal/Services/ConfigurationExportService.cs#L49): Portal configuration export reads every group, user, membership, folder, ACL, report, dataset, subscription, alert, and refresh job into memory before composing one large script string. Large portals can produce oversized memory spikes and slow export endpoints.
  - *Solution*: Stream export sections with `IAsyncEnumerable`/chunked response support and use lookup dictionaries for joins instead of repeated in-memory scans.
...
- [ ] **Recursive File Command Enumeration Materialization** — [SyncDirectoryStatementHandler.cs:L85](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.Engine/Handlers/SyncDirectoryStatementHandler.cs#L85) & [FileFunctions.cs:L176](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.Engine/Functions/FileFunctions.cs#L176): `SYNC DIRECTORY` and `FILE_LIST(..., recursive)` use `Directory.GetFiles(...)`, materializing full file arrays before processing. Large source trees or network shares can spike memory, block the engine thread, and delay cancellation.
  - *Solution*: Use `Directory.EnumerateFiles(...)` with streaming depth validation, cancellation checks, and bounded batching for metadata/result rows.
- [ ] **Dataset Storage Reconciliation Full Catalog Scan** — [DatasetStorageMaintenance.cs:L28](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.ReportPortal/Services/DatasetStorageMaintenance.cs#L28): Startup dataset maintenance loads all dataset rows, builds a full referenced-file set, and scans the dataset root. At 10k+ cached datasets this can slow portal startup and contend with user requests on shared storage.
