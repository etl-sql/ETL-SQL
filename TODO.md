...
- [x] **Priority 4: Remaining Select Materialization Boundaries** — Non-blocking select stages now preserve streams, while blocking DISTINCT, percentage counting, ties, and full ordering use bounded spill/replay paths.
  - [x] Pending external aggregate/window streams feed directly into `ExternalSortEngine.SortStreamAsync` for full `ORDER BY` instead of calling `ToListAsync()` before the spill sort.
  - [x] Streaming join output now feeds hybrid downstream aggregate/window spill decisions instead of materializing the entire joined rowset first.
  - [x] A hybrid direct-source `ORDER BY` path buffers only to the sort threshold, then replays the prefix into external sort rather than collecting all source rows before deciding to spill.
  - [x] Projected `DISTINCT` rows use structured compound keys and hybrid hash-partition spill instead of an unbounded delimiter-based string set.
  - [x] External `TOP PERCENT` uses encrypted count/replay spill and `WITH TIES` streams using only the boundary sort keys.
  - [x] `SelectPipelineBenchmarks` covers full external `ORDER BY`, join-to-aggregate/window, high-cardinality `DISTINCT`, `TOP PERCENT`, and `WITH TIES`.
...
- [x] **Priority 8: Orchestrator Queue Wakeups and HA Validation** — Added configurable poll backoff/jitter, provider-neutral shared throttle storage, leased remote-node slots, and PostgreSQL cross-instance contention verification. Provider notifications remain an optional future latency optimization.
  - *Progress*: SQLite cross-process throttle waits now use configurable exponential backoff and jitter instead of fixed 500 ms polling.
  - *Progress*: PostgreSQL HA nodes now coordinate through the configured shared store; active slots heartbeat and abandoned remote-node slots expire.
  - [x] PostgreSQL fault-injection coverage proves heartbeat renewal prevents expiry for long-running jobs and abandoned remote-node slots are reclaimed after lease expiration.
- [x] **Final Performance and Compatibility Gate** — Categorized correctness, integration, performance, Smoke/Standard certification, and timestamped BenchmarkDotNet lanes pass; no overlapping benchmark regressed beyond the 15% threshold.
  - [x] Benchmark coverage includes parser/script scale, SELECT shapes, TPC-H, external spill operators, snapshots/report rendering, and orchestrator scheduling/job execution.
  - [x] Published timing, allocation, working-set, spill-byte, and retained-row results in `Docs/Operations/v0.13.0-performance-results.md` from immutable timestamped artifacts.
...
- [x] **Select Pipeline Materialization Boundaries** — [SelectExecutionEngine.cs:L91](file:///C:/Users/chuck/scratch/ETL-SQL/src/ETL-SQL.Engine/Engines/SelectExecutionEngine.cs#L91): Large-query joins, aggregate/window results, full ORDER BY, QUALIFY, LIMIT, final projection, DISTINCT, TOP PERCENT, and WITH TIES now retain streaming or bounded spill boundaries.
  ...
  - *Completed*: Full external `ORDER BY`, join-to-blocking-stage handoff, direct-source spill sorting, high-cardinality `DISTINCT`, `TOP PERCENT`, and `WITH TIES` now have bounded paths and benchmark coverage under Priority 4.
