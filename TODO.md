...
- [~] Subscriptions for identity-sensitive reports: delivery re-executes the report script (not the snapshot) under no interactive identity, so it would fail-closed and email an **empty** report. Now refused with a clear ledger reason instead of silent blank delivery. Shared scan extracted to `Core.Governance.RowLevelSecurityScan`. *(Full per-recipient identity delivery — resolve recipient email → user → identity, thread through `IScriptExecutor`/`ExecutionSession` — remains; it is a cross-cutting change to the orchestrator execution abstraction.)*
