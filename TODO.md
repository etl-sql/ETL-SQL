...
- [~] A repository-wide security review finds no direct sensitive operation that bypasses the shared authorizer. *(Engine statement-handler filesystem surface reviewed and fully routed through `FileSystemPolicyAuthorizer`; remaining direct writes are fixed app-config (`GenerateJwtSecret` → `appsettings.json`), engine-managed session paths with sanitized names (`CreatePortalSubscription` → `SessionRoot/subscriptions`), or engine-owned spill/session (exempt by design). Found and fixed a real gap: file-connector data sources validated writes with the local `SecurityService` guardrail only — the enterprise `FileSystemPolicyAuthorizer` is now re-applied at the write boundary of **all six** file connectors (FlatFile/CSV, JSON, XML, Parquet, Excel, Avro) via a shared `FileConnectorPathHelper.AuthorizeWrite`, so a connection created before policy tightened (or via a deferred/`${placeholder}` target) cannot write outside approved roots (`FileConnectorWriteBoundaryTests`). Remaining follow-on: the file-connector **read** boundaries (defense-in-depth; base path is authorized at CREATE CONNECTION).)*
