...
- [x] Define policy-refresh semantics for work already running: security revocation and expired policy fail promptly; ordinary limit changes apply no later than the next operation boundary. *(Shared `OperationPolicyBoundary.Refresh` implements the contract and is invoked by the filesystem authorizer and the file-op-count/recursion-depth ceilings; 3.4/3.5 authorizers must reuse it.)*
...
- [~] Route all script-driven reads, writes, deletes, moves, copies, archive extraction, directory enumeration, spill, export, snapshot, and artifact paths through one canonical path-authorizer. *(Routed through `FileSystemPolicyAuthorizer`: file/directory ops, sync, transfer, zip extraction (pre-existing); now also BULK INSERT, SPLIT/MERGE FILES, CONVERT FILE ENCODING, VERIFY FILE INTEGRITY, WAITFOR FILE, EXPECT SCHEMA, EXPORT/PUBLISH DATASET, LINEAGE exports, CREATE LINEAGE, RUN SCRIPT/EXECUTE script reads, CREATE TEMPLATE/THEME, SET TEMPLATE_PATH, SSH/PGP key-pair output, bundle keyfile/source/export paths, and FILE_* / DIRECTORY_* scalar+table functions. Slice 2 added CREATE/ALTER CONNECTION file-based roots (Enumerate at registration; `${placeholder}` targets defer to use-time resolution), ALTER/DROP report-object template/theme mutations, and portal-subscription script paths. End-to-end enforcement suite `FileSystemPolicyEnforcementTests` proves enterprise-root deny/allow (incl. connection roots) plus unenrolled-standalone freedom. Remaining: engine-owned spill/cache policy limits (bullet 5 covers separation).)*
- [x] Enforce approved roots, read/write distinctions, maximum recursive depth, file-operation count, extension/type restrictions, and protected application/system paths. *(Approved roots + read/write distinctions enforced by `FileSystemPolicyAuthorizer`; `Security:MaxFileOperationsPerScript` and `Security:MaxRecursiveNestingDepth` enforced at the evaluator operation boundaries via shared `OperationPolicyBoundary` — immune to local `SET ALLOW_*` overrides, structured denials, policy-freshness re-check. New governed `Security:AllowedWriteExtensions` (schema `Filesystem.AllowedWriteExtensions`) denies mutating writes whose canonical target extension is outside the allowlist. Protected application/system paths remain local `SecurityService` sensitive-path enforcement, which the enterprise approved-roots gate is strictly narrower than — recorded as covered-by-composition rather than a separate governed key.)*
- [~] Resolve canonical targets before access and prevent bypass through `..`, relative paths, mixed separators/case, UNC/device paths, alternate data streams, symbolic links, junctions, hard links, and archive traversal. *(The authorizer canonicalizes via `GetFullPath` + per-component symlink/junction resolution and now rejects Windows non-canonical forms outright: `\\?\`/`\\.\`/`\??\` namespace prefixes, NTFS alternate-data-stream syntax, and trailing dot/space segments (Win32 open-time stripping bypass). `..` is rejected pre-resolution by `SecurityService.ValidatePath`; archive traversal is guarded by `SafeZipExtractor` with tests. Remaining: hard links are undetectable by path inspection (document as an OS-level boundary); Linux-side link-race hardening pairs with the TOCTOU re-check bullet below.)*
...
- [x] Keep engine-owned spill/cache paths separate from user-selected destinations while applying explicit policy limits to both. *(Spill/cache paths are engine-owned under `SpillStore` (session temp root), never user-selected, so they cannot collide with script destinations. The new governed `Security:MaxSpillBytesPerScript` (schema `Execution.MaxSpillBytesPerScript`) bounds cumulative engine spill volume: enforced at every `SpillStore` telemetry increment via `OperationPolicyBoundary.EnforceSpillCeiling` (reads the captured snapshot directly — spill is a per-write hot path; security revocation still enforced at operation boundaries). User-selected write destinations are bounded by the file-operation-count ceiling and approved roots above. End-to-end tests cover both limits.)*

#### 3.4 Network and connector enforcement
- [~] Enforce connector allowlists and destination host/port/scheme rules before DNS resolution and connection creation. *(New `ConnectorPolicyAuthorizer` (Core Governance, reuses `OperationPolicyBoundary.Refresh`) enforces `Connectors:AllowedTypes` and `Security:AllowedHosts` from the enrolled snapshot before the data source is created, wired into CREATE/ALTER CONNECTION ahead of `CreateDataSource`. Structured `ConnectorPolicyDeniedException`. `Security:AllowedHosts` was defined but previously enforced nowhere. Remaining: per-port/scheme rules and applying the same gate to the discovery/probe paths of individual connectors — see bullets below.)*
...
- [~] Apply the same authorization to REST, database, email, SFTP, object storage, remote policy/vault access, and connector-specific discovery/probe operations. *(REST — the top per-request SSRF vector (URL_TEMPLATE data-driven URLs, OAuth TOKEN_URL, NEXT_URL pagination, redirect hops) — now applies the enterprise host/internal-range gate (`ConnectorPolicyAuthorizer.EnforceEnterpriseHost`) at every request/redirect/pagination point, alongside its existing local `ValidateHost`. So a REST connection cannot follow a template/redirect/pagination URL into a denied host or an obfuscated internal range even under a wildcard allowlist. Other network connectors (DB/SFTP/object-storage/email) validate host at their probe points via local `ValidateHost`; their destination is authorized at connection creation, so routing those probe calls through the enterprise gate for refresh-currency is the remaining follow-on.)*
- [~] Ensure aliases, plugins, saved connections, and connection-string forms cannot bypass connector classification or destination checks. *(Saved connections restored via `LoadSessionState` now re-authorize against current policy before rehydration — a connection saved under a looser policy is dropped (with a warning) rather than reused to reach a now-denied type/destination. CREATE/ALTER CONNECTION already gate the connection-string forms. Remaining: connector-specific discovery/probe calls (e.g. `REMOTE_FILE_LIST`, `GetVersionAsync`) still call the local `SecurityService.ValidateHost` rather than the enterprise connector gate — those operate on already-authorized named connections, so the destination was authorized at creation, but re-routing them through `ConnectorPolicyAuthorizer` for policy-refresh currency is a follow-on.)*

#### 3.5 Process, Docker, resource, and script-setting enforcement
- [~] Gate external executables, arguments, working directories, environment inheritance, shell invocation, Docker images/registries, mounts, networks, privilege flags, and host access. *(Docker: new governed `Security:AllowedDockerImages` (schema `Process.AllowedDockerImages`) gates `USE DOCKER(...)` at the single `DockerStatementHandler` choke point before the container starts — supports exact `repo:tag`, tagless `repo` (any tag), and `prefix/*` registry/namespace wildcards, with registry-port-vs-tag disambiguation. Enforced even under WHAT IF, via `ProcessPolicyRules.EnforceDockerImage` (reuses `OperationPolicyBoundary.Refresh`). Remaining: the engine has no general external-executable/shell statement today, so that surface is not yet applicable; Docker mounts/networks/privilege flags are not currently script-settable (the run takes only image+alias) — revisit if those options are added.)*
- [~] Enforce parallelism, recursion, file-operation, email, string/result, memory/spill, execution-time, and other governed resource ceilings at runtime. *(Enforced at their runtime boundaries against the enterprise snapshot: file-operation count and recursion depth (evaluator operation boundaries), memory/spill bytes (`SpillStore` increments), and outbound email (`RecordSmtpEmailSend`, new governed `Security:MaxSmtpEmailsPerScript`). Parallelism is enforced via the SET ceiling. Remaining: string/result-size and execution-time have no governed policy key yet (local `SecurityService` limits only) — add keys or record non-enforceable at the completion gate.)*
...
- [x] Every governed key maps to a named enforcement boundary or is removed from the policy schema as non-enforceable. *(Living audit: `GovernanceEnforcementAuditTests` classifies all 24 `GovernancePolicyRegistry` keys — **14 enforced against the execution-policy snapshot** at named boundaries (mutation guardrails via `MutationGuardrailPolicy.Enforce`; `Security:AllowedExecutionModes` and `Security:RemoteExecutionMode` via `OperationPolicyBoundary.EnforceAllowedExecutionMode`/`EnforceRemoteExecutionMode` at execution start) and **10 via config-precedence** local/Portal boundaries. **Zero gaps** — every governed key names a boundary. The `RemoteExecutionMode` boundary is dormant only because no path produces a `ScriptExecutionMode.Remote` snapshot yet; it is tested directly and enforces the moment one does. The audit test fails on any unclassified/stale key or gap-set drift, so a new governed key cannot ship unenforced.)*
- [~] A repository-wide security review finds no direct sensitive operation that bypasses the shared authorizer. *(Engine statement-handler filesystem surface reviewed and fully routed through `FileSystemPolicyAuthorizer`; remaining direct writes are fixed app-config (`GenerateJwtSecret` → `appsettings.json`), engine-managed session paths with sanitized names (`CreatePortalSubscription` → `SessionRoot/subscriptions`), or engine-owned spill/session (exempt by design). Found and fixed a real gap: file-connector data sources validated writes with the local `SecurityService` guardrail only — the enterprise `FileSystemPolicyAuthorizer` is now re-applied at the write boundary of **all six** file connectors (FlatFile/CSV, JSON, XML, Parquet, Excel, Avro) via a shared `FileConnectorPathHelper.AuthorizeWrite`, so a connection created before policy tightened (or via a deferred/`${placeholder}` target) cannot write outside approved roots (`FileConnectorWriteBoundaryTests`). The same re-check is now also applied at every file-connector **read** boundary (`FileConnectorPathHelper.AuthorizeRead` in each `ReadBatches`), so neither reads nor writes through a file connection can escape the approved roots regardless of how the path was resolved. Both directions are covered by `FileConnectorWriteBoundaryTests`.)*
- [~] Bypass suites cover Windows and Linux paths, links, DNS/redirect behavior, connector aliases, child processes, Docker mounts, script overrides, and concurrent policy refresh. *(Covered: Windows non-canonical paths/ADS/device-prefix + symlink/junction substitution race (`FileSystemPolicyAuthorizerTests`); `..`/traversal + cross-platform symlink resolution (`SecurityService`); DNS/IP-literal/private-range + REST redirect/pagination (`NetworkDestinationRulesTests`, `ConnectorPolicyEnforcementTests`); saved-connection reuse (`ConnectorPolicyEnforcementTests`); script overrides / SET-can't-weaken (`ResourceCeilingEnforcementTests`); Docker image allowlist (`ProcessPolicyRulesTests`); **concurrent/mid-execution policy refresh — revocation (terminal) and tightening (recapture) (`PolicyRefreshBypassTests`)**. Remaining cells are boundary-not-yet-present rather than untested: Linux-only path forms (the authorizer's non-canonical rejections are Windows-specific by design; CI runs on Windows), child processes (spawned runners re-derive policy from machine enrollment at startup — determinism by re-derivation, not an in-process bypass), and Docker mounts/networks/privilege flags (not script-settable — `USE DOCKER` takes only image+alias).)*
- [x] Existing standalone tests prove no enterprise endpoint, certificate, cache, or organization restriction is required when unenrolled. *(`StandaloneRegressionTests` consolidates the proof: the default runtime is `Standalone` with no policy document/version/config and `IsEnrolled=IsAvailable=false`; an unenrolled snapshot imposes no governed values and every snapshot-boundary helper (`EnforceCeiling`/`EnforceSpillCeiling`/`EnforceAllowedExecutionMode`/`EnforceDockerImage`) short-circuits without throwing; and a battery of otherwise-governed operations (SET ceilings, persistent DELETE/INSERT without what-if/transaction) runs with no organization restriction. Per-domain `Standalone_Unenrolled` tests in the filesystem/connector/mutation/resource suites remain as targeted coverage.)*
...
- [~] Capture host/server utilization, distinct from per-job process cost. **Done:** free disk on state/spill volumes (`NodeCapacitySnapshot`, `DriveInfo`) alongside host memory-load % and CPU %; **persisted time series** — `HostMetrics` table + `IHostMetricsStore` (append/get/prune) sampled every heartbeat, pruned on the maintenance cycle (`Orchestrator:HostMetricsRetentionDays`, default 14); **read surface** — `SHOW HOST METRICS [nodeId] [INTO #t]` returns the last 24h newest-first (mirrors `SHOW JOB HISTORY`), usable in capacity-report scripts. *(store test + end-to-end SHOW test)* **whole-host CPU** — `NodeCapacityMonitor` now samples OS-level CPU (Windows `GetSystemTimes` / Linux `/proc/stat`, dependency-free) into `NodeCapacitySnapshot.HostCpuPercent`, threaded through the heartbeat into `HostMetricSample.HostCpuPercent` (first sample null until an interval exists; best-effort null on unsupported platforms). *(2 capacity-monitor tests)* All host-utilization capture is now done.
...
- [ ] Complete threat-model and senior security review with all high-severity findings resolved. *(Human activity. The enforcement work provides the material: the `GovernanceEnforcementAuditTests` key→boundary map, the bypass suites (`FileSystemPolicyAuthorizerTests`, `ConnectorPolicyEnforcementTests`, `NetworkDestinationRulesTests`, `ProcessPolicyRulesTests`, `MutationGuardrailEnforcementTests`, `PolicyRefreshBypassTests`, `FileConnectorWriteBoundaryTests`), and the `StandaloneRegressionTests`. Known non-blocking residuals for the reviewer: `Security:RemoteExecutionMode` has no trigger point yet (no execution path produces `ScriptExecutionMode.Remote`); hard links are undetectable by path inspection (OS-level boundary, documented).)*
...
- [x] Add delete tombstones, update delta segments, and compaction only after append-only storage and
  ...
  *(Native DELETE now records grant-accounted per-segment tombstone bitmaps for predicates supported by
  ...
  retain the established row-store downgrade. Best-effort compaction rewrites tombstoned segments once
