...
- [x] **P1 — Add a desktop host lifecycle and multi-project session contract**: Keep automatic
  ephemeral-port allocation and isolate each project host from every other project host.
  - [x] Add an explicit **Exit Studio** action that checks for dirty documents and active runs,
    requests graceful host shutdown, waits for a bounded timeout, and reports whether the process
    actually stopped.
  - [x] Track connected browser windows with renewable heartbeats. Treat `pagehide` or `sendBeacon`
    as advisory only; an ordinary tab close or browser crash must not be the sole shutdown signal.
  - [x] Add configurable idle shutdown after the last client disconnects when no run is active and
    no unsaved server-side draft remains.
  - [x] Persist a per-project local session record containing the normalized workspace root, PID,
  ...
  - [x] Make `etlsql studio <project>` reconnect to the healthy host already serving that project or
    start a new host on an OS-assigned port when none exists.
  - [x] Support simultaneous windows for different projects with separate hosts, ports, execution
    state, connection state, Git state, and filesystem boundaries. Closing or stopping one project
    must not affect another.
  - [x] Add `etlsql studio --new-window` to open another browser window against the existing project
  ...
  - [x] Add `etlsql studio list`, `etlsql studio open`, and `etlsql studio stop` lifecycle commands
...
- [x] **ETL-SQL Studio create connection doesn't work** I tried to create a connection using the connection wizard for MOCKDB
