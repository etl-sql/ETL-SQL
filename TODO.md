...
- [ ] Finish one managed-connection lifecycle across Engine, Portal, and Orchestrator:
      `CREATE/ALTER/TEST/SELECT/DROP CONNECTION` inside `EXECUTE <admin_conn> BEGIN ... END` must
      use the same catalog contract, connector metadata, authorization checks, secret-reference
      validation, redaction, impact analysis, enable/disable behavior, and audit vocabulary.
