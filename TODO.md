...
## v0.10.0 work
- [ ] **Installer parity on Linux and macOS**  The Windows MSI overhaul (Windows-service hosting, working-dir anchoring so logs/db/snapshots land beside the app, JWT secret bootstrap, install-folder security safe zone, pinned service ports 5001/5002, full payload incl. wwwroot, connection URLs surfaced, opt-in data cleanup on uninstall) needs equivalents in the Linux `.deb`/systemd build (`scripts/build_linux_packages.sh`, `src/ETL-SQL.Installer/linux/*.service`) and the macOS `.dmg` (`scripts/build_mac_dmg.sh`). Confirm the portal serves its UI, the JWT secret is generated, data writes are permitted, and both services start on each platform.
