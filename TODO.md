...
- [~] Enforce approved roots, read/write distinctions, maximum recursive depth, file-operation count, extension/type restrictions, and protected application/system paths. *(Approved roots + read/write distinctions enforced by `FileSystemPolicyAuthorizer`; `Security:MaxFileOperationsPerScript` and `Security:MaxRecursiveNestingDepth` now enforced at the evaluator operation boundaries via shared `OperationPolicyBoundary` — immune to local `SET ALLOW_*` overrides, with structured denials and policy-freshness re-check. Extension/type restrictions and protected app/system paths remain local `SecurityService` enforcement — no governed key exists in policy schema v1.0; add keys or record as non-enforceable at the completion gate.)*
