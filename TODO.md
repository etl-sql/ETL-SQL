...
- [ ] Close the remaining managed-connection parity gaps across Engine, Portal, and Orchestrator:
      `SHOW CONNECTION CONFIG`/catalog inspection, impact analysis, configuration export/import,
      fail-closed audit vocabulary, and authorization/redaction behavior must use one catalog
      contract across local engine execution and `EXECUTE <admin_conn> BEGIN ... END`.
