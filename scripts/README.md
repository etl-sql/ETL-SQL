...
| **[`test-lane.ps1`](./test-lane.ps1)** / **[`test-lane.sh`](./test-lane.sh)** | PowerShell / Bash | Cross-platform | The test suite gateway to run individual test lanes (smoke, fast, engine, portal, integration, perf, release, full, benchmarks, slt) with optional coverage mapping. The `perf` lane runs tagged engine hardening tests plus the dedicated perf project; the `fast`, `portal`, and `full` lanes also run the Node lineage UI smoke test. |
