...
| **[`Test-AllSamples.ps1`](file:///c:/Users/chuck/scratch/ETL-SQL/scripts/Test-AllSamples.ps1)** | PowerShell | Windows / macOS / Linux | Discovers and executes all samples (`*.etlsql` and `*.rptsql`) in the `samples/` folder to validate engine runtime backward compatibility. |
| **[`Compare-Benchmarks.ps1`](file:///c:/Users/chuck/scratch/ETL-SQL/scripts/Compare-Benchmarks.ps1)** | PowerShell | Windows / macOS / Linux | Compares BenchmarkDotNet JSON results against a checked-in baseline file and returns an exit code of 1 if regression exceeds a given threshold. |
...
| **[`build_vsix.ps1`](file:///c:/Users/chuck/scratch/ETL-SQL/scripts/build_vsix.ps1)** | PowerShell | Windows / macOS / Linux | Builds and packages the VS Code extension bundle into a publish-ready `.vsix` file using `vsce`. |
| **[`publish_vsix.ps1`](file:///c:/Users/chuck/scratch/ETL-SQL/scripts/publish_vsix.ps1)** | PowerShell | Windows / macOS / Linux | Publishes the packaged VS Code `.vsix` bundle to VS Code Marketplace and Open VSX Registry. |
...
| **[`test_sdk_build.ps1`](file:///c:/Users/chuck/scratch/ETL-SQL/scripts/test_sdk_build.ps1)** | PowerShell | Windows / macOS / Linux | Verifies compilation of self-contained single-file SDK CLI tools and runs validation smoke tests on them. |
...
### 2.4 Running SQLite Logic Tests (SLT) Corpus
...
### 2.5 Regenerating the Syntax Index Token Inventory
