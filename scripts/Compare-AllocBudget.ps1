...
Write-Host ("     bytes/row {0:N1} (budget {1:N1}); GC gen2 {2} (budget {3}); pause {4:N0} ms (budget {5:N0}); peak WS {6:N0} MB (budget {7:N0})" -f `
...
if ($new.allocation.bytesPerRow -lt $base.allocation.bytesPerRow * 0.9) {
    Write-Host "     Run is >10% better than budget — consider re-blessing: .\scripts\Test-SpillAllocProfile.ps1 -Rows $($new.rows) -UpdateBudget" -ForegroundColor Cyan
}
exit 0
