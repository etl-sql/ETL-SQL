...
**Theme:** every graphical visual is rendered by ETL-SQL's own native engine — no external chart runtime — and ETL-SQL Studio arrives as an Alpha alongside the existing authoring tools.

### Breaking Changes

- Data-quality rules moved out of comment tags into first-class syntax. A column declares `EXPECT <rule> [ON FAILURE THROW | WARN | QUARANTINE]`, replacing `/* @expect: '…'; @fail: '…'; */` and the numbered `@expect_N`/`@fail_N` pairing. Writing a rule as a tag is now a lint error naming the clause to use instead.
- `ASSERT JOB` folds onto the same failure-action vocabulary. Stacked `ON FAILURE WARN | NOTIFY <notification> | THROW` blocks replace `ON CRITICAL_FAILURE THROW` and `WITH (FAIL_ON_WARN = TRUE)`.
- The data-quality rule catalog's `rule_tag` column is renamed `rule_clause`, with values `EXPECT`/`EXPECT #2` instead of `@expect`/`@expect_1`. This affects `eng.data_quality_rules`, `SHOW DATA QUALITY RULES`, and the Portal's data-quality views.
- Browser manifest delivery no longer serializes `chartSpec`, `chartData`, `plotPlan`, `microCharts[].plotPlan`, or the legacy `interactions` map. Read `visual.interaction` and `visual.nativeSvg`; a tested compatibility shim keeps pre-v0.19.0 snapshots rendering.
- An unrecognised `SET REPORT` key is now a syntax error naming the supported keys, instead of parsing through the arbitrary-identifier path and being silently discarded.

### Added

- **ETL-SQL Studio (Alpha)** — a unified dual-projection visual and script workbench across the SaaS Portal and a desktop host (`etlsql studio`), with document tabs, an activity rail, canvas/split/code projections, and CodeMirror 6 sub-line execution. Studio does **not** replace Report Builder or Workstation Editor in this release, and nothing is deprecated.
- **Studio authoring surfaces** — distinct Dashboard and Paginated Report workflows, a document outline, a data-model view, an engine panel carrying scope and `EXPLAIN`, a visual formatting inspector, a filter pane that reaches every value in a column, cross-visual filter and cascade authoring, a Git diff viewer, a workspace Explorer, and PDF export with pagination preview.
- **Studio pipeline canvas** — a real engine-backed DAG with control-flow containers (`PARALLEL`, `FOREACH`, transaction scopes), conditional precedence edges, a positional scope inspector, and Run to Selected Node with a confirmation naming every write that would outlive the run.
- **Native Grammar-of-Graphics completion** — typed field/`DATUM`/`VALUE` bindings, inherited encodings, explicit stack/offset/interval placement, deterministic jitter and nudge, continuous colour ranges, wrapped facets, fixed Cartesian aspect, and category-local `TICK` marks across parser, formatter, lint, LSP, lineage, native SVG, terminal, accessibility, and export paths.
- **Chart property parity** — the last gaps against Power BI, Tableau, and ggplot2: axis titles, bounds, ticks and label rotation; gridline and spine styling; 100% stacking; error bars; forecast and anomaly encodings; reference lines and bands; running-total and percent-of-total overlays; series and data labels with leader lines; marker shape, stroke and size; line width and dash; step interpolation; legend controls; plot-area styling; and axis typography.
- **Chart interaction and delivery** — `ChartSpec.InteractionSpec` as the one interaction contract, `EMIT_FILTER (TARGETS = ...)` for send-side cross-filtering, `TOOLTIP (FIELDS = (...))` field-list tooltips, `OPEN_URL(TEMPLATE = ..., PARAMS = (...))` row-driven URLs, `SHOW_EXPORT`, `SHOW_DATA_VIEW`, `ZOOM_GROUP` linked zoom, and `SAMPLING`/`PROGRESSIVE` for dense series.
- **Constrained `HTML` visuals** — escaped typed bindings, scoped CSS, declarative actions, and bounded `VISUAL(...)` embeds with `SPARKLINE(...)` and `PROGRESS_BAR(...)` helpers, under explicit row, node, byte, query, and render budgets, with a deterministic semantic fallback for PDF, Markdown, terminal, email, and screen readers.
- **Offline report packages** — `etl-sql-report offline <script>` turns an `.etlsnap` package into a single self-contained HTML file that opens with no server and no network, under a `default-src 'none'` Content-Security-Policy.
- **Report properties** — `SET REPORT TIME_ZONE`, `SET REPORT LOCALE`, and `SET REPORT NULL_LABEL` with deterministic server-side precedence, plus `Scheduler:DefaultTimeZone`, `Reporting:DefaultLocale`, and `Reporting:DefaultNullLabel` configuration. Nothing is inferred from the viewer's browser, so browser, PDF, email, and terminal renderings of one report agree.
- **Report design tokens** — `CREATE STYLE PALETTE = (...)` sequences resolving into scoped `--etl-*` variables with deterministic inheritance, safe CSS serialization, stable series-colour assignment, explicit-colour precedence, and contrast validation.
- **Connection Wizard** — code-first connection authoring across Report Builder, Portal Admin, Workstation Editor, VS Code, and LSP, with approved Gateway resource discovery and binding, live zero-trust path feedback, and canonical script generation.
- **Reporting golden lane** — one directory-discovered lane covering both visual catalogs, pinning the serialized `PlotPlan`, the native SVG, the semantic fallback, and the terminal render per fixture, with named-visual-to-`CUSTOM` translation equivalence and enforced cross-platform determinism.
- **Browser payload budget** — raw and gzip ceilings for the shared runtime and end-to-end page weight, gated on every default test run and re-blessed only through `Test-ReportPayloadBudget.ps1 -UpdateBudget`.
- **Browser type gate** — a root `tsconfig.json` with `checkJs`, `.d.ts` contracts generated by reflection over the C# DTOs and enums, and a zero-finding ratchet wired into pre-push and CI.
- **Workload identity federation** — short-lived, audience-bound identity exchange for GitHub, GitLab, Azure DevOps, and `private_key_jwt` schedulers, with durable replay rejection and owner-capped service tokens, making CI and scheduled workloads secretless.
- **Gateway ambiguous-write triage** — durable cases per tenant and operation with acknowledgement, assignment, evidence, notes, four externally verified resolutions, and append-only event history, so a retry cannot duplicate an uncertain write.
- **Release gate hang protection** — a per-phase watchdog kills a stalled pre-release phase on a stall or hard cap and reports why, phase logs stream live rather than buffering until completion, and a preflight clears leftover browsers and test hosts and proves the Docker daemon answers before the Docker lane runs.

### Changed

- Split the canonical Studio browser implementation into responsibility-owned modules for host contracts, document state, data sampling, Report-SQL mutations, save security, and lease lifecycle, and lifted the Portal's inline `<script type="module">` blocks into `wwwroot/js/pages/`.
- Focused layout modules (`TREEMAP`, `SUNBURST`, `SANKEY`, `NETWORK`, `MAP`, `MATRIX`) now consume shared theme tokens, series colours, accessible names, and resolved interaction metadata instead of a private colour array on a hardcoded 600×350 canvas.
- Chart date and time values are rendered by the report formatter rather than reused from the engine's generic row strings, so the same report no longer renders different instants on two hosts.
- Reconciled the reporting capability matrix with the source-backed renderer contract: every graphical catalog visual uses the shared native `PlotPlan` path or an approved focused native SVG module, and none requires ECharts, ClearScript, or another external chart runtime.

### Fixed

- **The Windows installer honours the component properties it is given, and no longer fails when the Portal is slow to start.** `DEPLOY_PROFILE` defaulted to `ENTERPRISE`, so its preset ran on every install — including `/qn`, where there are no checkboxes to preset — and overwrote whatever the caller passed: `msiexec /i ETL-SQL.msi /qn INSTALL_ORCHESTRATOR=0` applied the property and then silently reset it 83 ms later, installing the Orchestrator anyway. It is now undefined by default, as its own documentation always said; the preset set every property to the value it already defaults to, so an ordinary install is unchanged. Separately, the installer waited for the Portal service to report *running* before finishing, but the Portal's first start creates and migrates `portal.db` — on a slow or cold machine that exceeds the Windows service start timeout, and the whole installation failed with error 1920 and rolled back a build whose files were all correct. The install no longer waits on that first start (the service is still `auto`, so Windows starts it); uninstall and upgrade still wait for the stop, where the file lock genuinely must be released.

- `CUSTOM` charts cross-filter on the column they are keyed on, receive the resolved theme and style tokens named visuals receive, honour author-supplied `Y_START`/`Y_END` and `X_START`/`X_END` on non-stacked `RECT` layers, anchor each diagnostic to the node carrying the mistake, and report every duplicate in a single pass.
- `||` string concatenation works across the full expression grammar — explicit aliases, parentheses, larger expressions, SQL-style `NULL` propagation, and dialect compilation.
...
- The offline snapshot branch no longer treats a viewer served over `http` as web mode, so it stops polling an API that is not there and detail popovers resolve from the manifest already in memory.

### Security

- Studio secret handling uses the same PBKDF2 + AES-GCM `ENC:` envelope as the Connection Wizard and engine; plaintext values never reach modal markup, and Base64 is never presented as encryption.
- `OPEN_URL` templates interpolate only the columns the author declared and URL-encode every value, so a row value cannot introduce a path segment, query parameter, or scheme of its own.
- Offline report packages declare `default-src 'none'`, so a self-contained file cannot quietly depend on being online.
- Verified Viewer Context is certified for SQL Server Gateway resources: signed viewer values installed through parameterized `SESSION_CONTEXT`, no claim-driven role selection, and pooled sessions cleared or evicted after success, provider failure, cancellation, timeout, and broken connections.
