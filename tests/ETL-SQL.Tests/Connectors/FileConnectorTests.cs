...
namespace ETL_SQL.Tests.Connectors
{
    ...
    public class FileConnectorTests : IDisposable
    {
        ...
        private string WriteEncryptedZip(string entryName, string content, string encryptedName, string password)
        {
            var sourcePath = Write(entryName, content);
            var zipPath = Path.Combine(_dir, entryName + ".zip");
            using (var zip = System.IO.Compression.ZipFile.Open(zipPath, System.IO.Compression.ZipArchiveMode.Create))
            {
                zip.CreateEntryFromFile(sourcePath, entryName);
            }

            var encryptedPath = Path.Combine(_dir, encryptedName);
            CryptoUtils.EncryptFile(zipPath, encryptedPath, password, overwrite: true);
            return encryptedPath;
        }

        private string WriteMinimalXlsx(string name)
        {
            var path = Path.Combine(_dir, name);
            ...
            AddEntry("xl/sharedStrings.xml",
                ...
                "</sst>");
            AddEntry("xl/worksheets/sheet1.xml",
                "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
        ...
        private string WriteZipFromFile(string zipName, string sourcePath, string entryName)
        {
        ...
        public async Task Excel_CompressedWorkbook_ReadsRowsColumnsAndTables()
        {
            var workbookPath = WriteMinimalXlsx("employees.xlsx");
            var zipPath = WriteZipFromFile("employees.xlsx.zip", workbookPath, "employees.xlsx");
            var opts = new Dictionary<string, string>
            {
                ["COMPRESS"] = "ON"
            };

            var ds = new ExcelDataSource(Ctx, zipPath, opts);

            var batches = await ds.ReadBatches().ToListAsync();
            var columns = (await ds.GetColumnsAsync()).ToList();
            var tables = (await ds.GetTablesAsync()).ToList();
