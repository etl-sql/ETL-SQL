...
This folder is the end-to-end "golden" Report-SQL workflow for ETL-SQL. It is intentionally self-contained so the same script can be used as a README walkthrough, preview target, Report Portal demo, export target, and regression fixture.
...
7. Publish: the script can be published directly to Report Portal.
...
For Report Portal, publish `samples\golden_workflow\golden_workflow.rptsql`, execute the report, change the region or margin controls, then export `OrderDetail` as CSV. Portal or automated runs can pass `@ExportPath` to put the generated staging CSV in a portal-safe temp location.
