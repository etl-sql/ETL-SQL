...
| Page | Description |
| :--- | :--- |
| [Advanced File Operations](advanced-file-operations.md) | Hub for the advanced filesystem statements, plus directory-connection path aliasing. |
...
| [MERGE FILES](merge-files.md) | Concatenates multiple files into a single destination file. |
...
| [SPLIT FILE](split-file.md) | Splits a larger text file into multiple chunk files by row count or byte size. |
| [SYNC DIRECTORY](sync-directory.md) | Mirrors a source directory to a destination directory. |
...
| [VERIFY FILE INTEGRITY](verify-file-integrity.md) | Computes file hashes and validates them against expected values. |
| [WAITFOR FILE UNLOCKED](waitfor-file-unlocked.md) | Blocks until a file arrives and is fully unlocked. |
