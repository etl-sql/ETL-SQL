...
| [Portal `eng.*` Inspection](portal-show.md) | Query Portal users, reports, sessions, permissions, audit, dependencies, and operational data inside an `EXECUTE portal` block with ordinary `SELEC... |
| [Portal SMTP Connections](portal-smtp.md) | Register a mail relay in the portal's governed connection catalog inside an `EXECUTE portal` block. SMTP is an ordinary connector, so it uses the o... |
| [Portal Report Subscriptions](portal-subscription.md) | Schedule report delivery through a `PORTAL` connection. |
| [Portal User Management](portal-user.md) | Manage portal user accounts, roles, and authentication tokens inside an `EXECUTE portal` block. |
| [revoke](revoke.md) | REVOKE removes portal permissions, share/embed tokens, or user authentication tokens. |
| [Service Accounts](service-accounts.md) | Service accounts are non-interactive Portal identities for unattended API and CLI work. An |
