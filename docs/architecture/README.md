...
| [ETL-SQL Deployment Profile Architecture](DeploymentProfiles.md) | **Status:** Approved target architecture; defines the common product, provider substitution, logical bindings, profile topology, state placement, and authority boundaries for Solo, Team, Enterprise, and SaaS |
...
| [ETL-SQL SaaS Tenant Isolation Architecture](SaaSTenantIsolation.md) | **Status:** Approved target architecture; tenant identity, control/data planes, hardened execution, durable checkpoints, secure outbound Gateway, capacity, telemetry, support, and isolation certification |
| [ETL-SQL Tenant Portability Architecture](TenantPortability.md) | **Status:** Approved target architecture; unified tenant bundle, state classification, rebinding, import, cutover, rollback, deletion, and SaaS exit |
