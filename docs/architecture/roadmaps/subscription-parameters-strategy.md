...
> [!IMPORTANT]
> **Historical feature plan.** Relative date parameters and subscription parameter behavior are now documented in current reference/user docs. Use this file for rationale and implementation history only; use `docs/reference/dates-times/reldate.md`, `docs/guides/report-sql.md`, and `docs/administration/portal/README.md` for current behavior.
...
- [ ] `docs/administration/portal/README.md` — update Section 8 (Subscriptions) with `CREATE SUBSCRIPTION` / `ALTER SUBSCRIPTION` syntax and PARAMETERS clause.
