...
| **Security-event outbox** | `…/<env>/data/security-events.db` via `ETLSQL_SECURITY_EVENT_OUTBOX_PATH` | dedicated path per environment | **The default is machine-wide** (`LocalApplicationData`), shared by every ETL-SQL process on the host. Leaving it unset puts two environments' security events into one queue — a cross-environment leak of exactly the records isolation exists to keep apart. Enrolled machines keep theirs beside the protected enrollment state and ignore the override. |
