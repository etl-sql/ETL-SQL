...
Host-level settings, including security limits, dashboard ports, and background service deployment (NSSM/systemd), are now managed in the central **[Administrators Guide](administration.md)**.
...
Assign the `OrchestratorManager` role to operations staff who need to manage jobs but should not be able to create users or manage reports. See the [Report Portal Administrator's Guide](report-portal-admin.md#orchestrator-manager-role) for role assignment instructions.
...
*For complete function and connector references, see [Standard Library](../reference/functions/standard-library.md) and [Data Connectors](../reference/connectors/data-connectors.md).*
